...
def criar(cliente: Cliente, admin=Depends(get_admin_user)):
    ...
    if _clientes_tem_colunas_geo(cur):
        cur.execute('''INSERT INTO clientes (nome, telefone, endereco, numero_casa, bairro, referencia, latitude, longitude)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s)''',
                    (cliente.nome, cliente.telefone, cliente.endereco, cliente.numero_casa, cliente.bairro,
                     cliente.referencia, cliente.latitude, cliente.longitude))
    else:
...
def editar(id: int, cliente: Cliente, admin=Depends(get_admin_user)):
    ...
    if _clientes_tem_colunas_geo(cur):
        cur.execute('''UPDATE clientes
                       SET nome=%s, telefone=%s, endereco=%s, numero_casa=%s, bairro=%s, referencia=%s,
                           latitude=%s, longitude=%s
                       WHERE id=%s''',
                    (cliente.nome, cliente.telefone, cliente.endereco, cliente.numero_casa, cliente.bairro,
                     cliente.referencia, cliente.latitude, cliente.longitude, id))
    else:
